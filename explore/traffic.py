...
#         self.combine_all_csv(data_dir + 'traffic_data/temp/', 'traffic_', 'traffic.csv')
    ...
    def __unittest(self):
        ...
        assert [0, 0, 0] == self.find_prev_traffic(
            pd.Series([1, '2016-01-01-2']), traffic_dict=traffic_dict, pre_num=3).tolist()
        assert [2246, 2081] == self.find_prev_traffic(
            pd.Series([1, '2016-01-01-9']), traffic_dict=traffic_dict, pre_num=2).tolist()

        data_dir = g_singletonDataFilePath.getTest1Dir()
        traffic_dict = self.get_traffic_dict(data_dir)
        assert [346, 424, 0] == self.find_prev_traffic(pd.Series(
            [66, '2016-01-30-141']), traffic_dict=traffic_dict, pre_num=3).tolist()
        assert [501, 484, 447] == self.find_prev_traffic(pd.Series(
            [66, '2016-01-30-70']), traffic_dict=traffic_dict, pre_num=3).tolist()
        assert [772, 802, 775] == self.find_prev_traffic(pd.Series(
            [57, '2016-01-24-58']), traffic_dict=traffic_dict, pre_num=3).tolist()
    ...
    def get_traffic_dict(self, data_dir):
        t0 = time()
        filename = '../data_raw/' + \
            data_dir.split('/')[-2] + '_traffic.csv.dict.pickle'
        ...
        for _, row in df.iterrows():
            resDict[tuple(row[['start_district_id', 'time_slotid']
                              ].tolist())] = row['traffic']
    ...
    def process_one_df(self, df):
        df['start_district_id'] = df['start_district_hash'].map(
            singletonDistricId.convertToId)
        df['traffic'] = df[['level_1', 'level_2', 'level_3']].apply(
            self.cal_traffic_value, axis=1,)
